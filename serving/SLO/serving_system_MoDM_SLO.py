...
parser.add_argument("--warm_up_size", type=int, default=0, required=False, help="number of warmup requests")
parser.add_argument("--cache_directory", type=str, required=False, help="directory of pre-cached images")
parser.add_argument("--image_directory", type=str, required=False, help="directory of generated images")
parser.add_argument("--req_rate", type=str, required=False, help="request rate / minute")
...
def terminate_all_processes():
    active_children = mp.active_children()  # Get all active child processes
    for child in active_children:
        print(f"Terminating process {child.pid}...")
        child.terminate()
        child.join()  # Wait for process termination
...
def request_scheduler(
    ...
    cached_image_paths, shared_new_images, lock,  model, processor,worker_status, scheduler_status, num_gpus, shutdown_event, warm_up_size=1000, cache_size=args.cache_size,  log_file="request_throughput_SLO_MoDM.csv"
):
    ...
    for i, row in sorted_df.iloc[warm_up_size:].iterrows():
        ...
        if new_images:
            ...
            if final_image_embeddings is not None:
                ...
                final_image_embeddings = torch.cat((final_image_embeddings, new_image_embeddings), dim=0)
                cached_image_paths.extend(batch_new_image_paths)
                assert len(cached_image_paths) == final_image_embeddings.shape[0], \
    f"Dimension mismatch: {len(cached_image_paths)} != {final_image_embeddings.shape[0]}"
        ...
        if final_image_embeddings is not None and final_image_embeddings.size(0) > cache_size:
            # Calculate the number of items to evict
            print("cache size:", final_image_embeddings.shape )
            print("maximum cache size:", cache_size)
            num_to_evict = final_image_embeddings.size(0) - cache_size
        ...
        if final_image_embeddings is not None and final_image_embeddings.size(0) >= warm_up_size:
            ...
            print("retrieved index:", best_index)
            print("cache size:", final_image_embeddings.size(0) )
            print("cache image path size:", len(cached_image_paths))
...
def global_monitor(request_rate_queue, control_queues, num_gpus, avg_latency_large, avg_latency_small, shutdown_event, total_num_steps=50):
    ...
    while True:
        try:
            ...
            if available_throughput <= hit_workload or num_large > num_gpus:
                print("Error: The number of GPUs is not enough for the request rate.")
                shutdown_event.set()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.cache_directory):
        print(f"The directory {args.cache_directory} does not exist.")
    ...
    if 'timestamp' in metadata_df.columns:
        ...
        rate_per_second = 60 / time_gap  if args.req_rate is None else (args.req_rate/60)# Poisson process rate (lambda)
    ...
    scheduler = mp.Process(target=request_scheduler, args=(
        hit_queue, nonhit_queue, selected_requests, start_time, request_rate_queue, final_image_embeddings, cached_image_paths, shared_new_images, lock, model, processor, worker_status,scheduler_status, num_gpus, shutdown_event, warm_up_size
    ))
    scheduler.start()

    # Start global monitor process
    monitor = mp.Process(target=global_monitor, args=(request_rate_queue, control_queues, num_gpus, avg_latency_large, avg_latency_small, shutdown_event))
    monitor.start()

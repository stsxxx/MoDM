...
filtered_df['seconds_from_start'] = filtered_df.groupby('seconds_from_start')['seconds_from_start'].transform(add_poisson_seconds)
...
save_path = f"final_image_embeddings_{init_size}.pt"

final_image_embeddings = torch.load(save_path)

print("Loaded embeddings shape:", final_image_embeddings.shape)

cache_prompt = [filtered_df['prompt'].iloc[i] for i in range(0,100000)]

print("size of embedding:", final_image_embeddings.shape[0])
print("size of prompt:", len(cache_prompt))
...
minute = 0
num_hit_min = 0
log_file=f"hit_rate_{cache_size}_tem.csv"
with open(log_file, "w") as f:
    f.write("timestamp, hit_rate, 5, 10, 15, 25, 30\n")
# Iterate over the new requests
for i in tqdm(range(0,  1100000)):  # Adjust range if necessary
# for i in tqdm(range(100)):  # Adjust range if necessary
    ...
    if highest_score not in similarity_count:
        similarity_count[highest_score] = 1
    else:
        similarity_count[highest_score] += 1
    ...
    if highest_clip_score.item() < 0.25:
        ...
        # if  final_image_embeddings.shape[0] > cache_size:
        #     final_image_embeddings = final_image_embeddings[1:]
        #     embedding_map = embedding_map[1:]
        #     retrieve_time = retrieve_time[1:]
        if start_time - record_time >= 60:
            elapsed_time = start_time - record_time
            minute += 1
            hr = num_hit_min / request_count
            rate_5 = k_distribution[5] / request_count
            rate_10 = k_distribution[10] / request_count
            rate_15 = k_distribution[15] / request_count
            rate_30 = k_distribution[30] / request_count
            rate_25 = k_distribution[25] / request_count
            k_distribution[5] = 0
            k_distribution[10] = 0
            k_distribution[15] = 0
            k_distribution[25] = 0
            ...
            with open(log_file, "a") as f:
                f.write(f"{minute},{hr}, {rate_5}, {rate_10}, {rate_15}, {rate_25}, {rate_30}\n")
            num_hit_min = 0
            request_count = 0
            record_time = start_time
        continue
    
    if highest_clip_score.item() >= 0.3:
        k = 30
        distribution[k] += 1
    elif highest_clip_score.item() >= 0.29:
        k = 25
        distribution[k] += 1
    elif highest_clip_score.item() >= 0.28:
        k = 15
        distribution[k] += 1
    elif highest_clip_score.item() >= 0.27:
        k = 10
        distribution[k] += 1
    elif highest_clip_score.item() >= 0.25:
        k = 5
        distribution[k] += 1
    k_distribution[k] += 1
    num_hit_min += 1
    ...
    print("k:", k)
    print(prompt[0])
    print("hit prompt:", cache_prompt[highest_clip_score_index])
    # final_image_embeddings = torch.cat([final_image_embeddings, image_embedding], dim=0)  
    # embedding_map.append(i)  
    # retrieve_time.append(0)  
    # if  final_image_embeddings.shape[0] > cache_size:
    ...
    # if retrieve_time[highest_clip_score_index] >= maximum_hit:
    #     num_5hit += 1
    #     final_image_embeddings = torch.cat((
    #         final_image_embeddings[:highest_clip_score_index],
    #         final_image_embeddings[highest_clip_score_index + 1:]
    #     ), dim=0)

    #     embedding_map = embedding_map[:highest_clip_score_index] + embedding_map[highest_clip_score_index + 1:]
    #     retrieve_time = retrieve_time[:highest_clip_score_index] + retrieve_time[highest_clip_score_index + 1:]
    if start_time - record_time >= 60:
        elapsed_time = start_time - record_time
        minute += 1
        hr = num_hit_min / request_count
...
# Show the plot
plt.show()
plt.savefig(f'time_differences_histogram_{cache_size}_{maximum_hit}.png', dpi=300, bbox_inches='tight')
...
# Show the plot
plt.show()
plt.savefig(f'retrieve_times_histogram_{cache_size}_{maximum_hit}.png', dpi=300, bbox_inches='tight')
